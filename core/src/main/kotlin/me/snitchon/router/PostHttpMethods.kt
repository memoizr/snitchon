...
object PostHttpMethods {
    fun <W : RequestWrapper,
            G : Group,
            P : ParametrizedPath<G, *>
            > Router<W, P>.POST(path: String = "") =
        postEndpoint<_, W>(prefix.group, path)
    ...
    fun <
            ...
            > Router<W, ParametrizedPath0>.POST(p: P) =
        postEndpoint<_, W>(prefix.group.with(p.group.p1), p.path)
    ...
    fun <
            ...
            P : ParametrizedPath1<PP, Ph>> Router<W, t>.POST(p: P) =
        postEndpoint<_, W>(prefix.group.with(p.group.p1), p.path)
    ...
    fun <G : Group, W : RequestWrapper> postEndpoint(group: G, path: String) = Endpoint<W, _, _, _>(
        EndpointMeta(
            HTTPMethod.POST,
            path,
            "",
